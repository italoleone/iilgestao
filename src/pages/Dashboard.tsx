...
import {
  DISCIPLINE_SHORT, STATUS_LABELS, type Discipline, type ProjectStatus,
} from "@/types";
import { FolderKanban, AlertTriangle, Clock, TrendingUp, ArrowRight, Loader2 } from "lucide-react";
...
export default function Dashboard() {
  ...
  const activeProjects = useMemo(() => projects.filter(p => p.status === "em_andamento"), [projects]);
  const lateProjects = useMemo(() => projects.filter(p => p.status === "atrasado"), [projects]);
  const totalSaleValue = useMemo(() => projects.reduce((s, p) => s + p.saleValue, 0), [projects]);
  ...
  const projectsByDiscipline = useMemo(() => {
    ...
    return Object.entries(counts).map(([key, value]) => ({
      name: DISCIPLINE_SHORT[key as Discipline], value, color: disciplineColors[key as Discipline],
    }));
  }, [projects]);

  const hoursChart = useMemo(() => {
    return projects.filter(p => p.status !== "concluido").map(p => ({
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <Card key={kpi.title} className="animate-reveal-up shadow-sm hover:shadow-md transition-shadow" style={{ animationDelay: `${(i + 2) * 60}ms`, animationFillMode: "backwards" }}>
              ...
              <CardContent>
                ...
                <p className={`text-xs mt-1 ${kpi.accent ? "text-destructive" : "text-muted-foreground"}`}>{kpi.sub}</p>
              </CardContent>
            </Card>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2 shadow-sm animate-reveal-up" style={{ animationDelay: "360ms", animationFillMode: "backwards" }}>
            <CardHeader><CardTitle className="text-base">Valor por Projeto</CardTitle></CardHeader>
            <CardContent>
              <div className="h-64">
                {hoursChart.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={hoursChart} barGap={4}>
                      ...
                      <Bar dataKey="vendidas" name="Valor Venda" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : <p className="text-sm text-muted-foreground text-center pt-20">Nenhum projeto ativo.</p>}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-sm animate-reveal-up" style={{ animationDelay: "420ms", animationFillMode: "backwards" }}>
            ...
            <CardContent>
              <div className="h-48">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={projectsByDiscipline} cx="50%" cy="50%" innerRadius={50} outerRadius={75} paddingAngle={4} dataKey="value">
                      {projectsByDiscipline.map(entry => <Cell key={entry.name} fill={entry.color} />)}
                    </Pie>
                    <Tooltip />
                  </PieChart>
                </ResponsiveContainer>
        ...
        <Card className="shadow-sm animate-reveal-up" style={{ animationDelay: "480ms", animationFillMode: "backwards" }}>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-base">Projetos Ativos</CardTitle>
            <button onClick={() => navigate("/projetos")} className="text-sm text-muted-foreground hover:text-foreground flex items-center gap-1 transition-colors">
          ...
          <CardContent>
            <div className="overflow-x-auto">
              ...
              {activeProjects.length === 0 && <p className="text-center py-8 text-muted-foreground">Nenhum projeto ativo.</p>}
            </div>
          </CardContent>
        </Card>
      </div>
